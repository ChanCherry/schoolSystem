...
public class moStuSelect1_servlet extends HttpServlet{
	protected void doPost(HttpServletRequest req,HttpServletResponse resp) throws ServletException, IOException{ 
		...
		String xing=req.getParameter("xing");//获取请求的参数
		String tno=req.getParameter("tno");
		String tname="";
		query query=new query();
		
		String queryString="select * from Stu where 姓名 like N'%"+xing+"_%'";
		ResultSet rs=query.executeQuery(queryString);
		int flag=0;
		out.println("<h2>查询姓“"+xing+"”的学生信息如下表：</h2>");
		out.print("<table width='600' border='1' cellspacing='0' border-collapse='collapse'><tr bgcolor='#BBFFFF'><td><center>学号</center></td><td><center>姓名</center></td>" +
		...
		try {
			...
			if(flag==1){
				...
				out.println("<br><br><a href='stuMohu.jsp?tno="+tno+"&tname="+tname+"'>返回模糊查询</a>");
				out.println("&nbsp;&nbsp;&nbsp;<a href='teaInfo.jsp?tno="+tno+"&tname="+tname+"'>返回教师中心</a>");
			}
			else{
				out.println("<script>alert('查询失败！列表里不存在姓“"+xing+"”的学生！')</script>");
				out.println("<script>window.location.href='stuMohu.jsp?tno="+tno+"'</script>");
			}
