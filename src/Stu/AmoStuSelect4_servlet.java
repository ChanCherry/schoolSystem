...
public class AmoStuSelect4_servlet extends HttpServlet{
	protected void doPost(HttpServletRequest req,HttpServletResponse resp) throws ServletException, IOException{ 
		...
		String sname=req.getParameter("sname");//获取请求的参数
		query query=new query();
		
		String queryString="select * from Stu where 学号='"+sno+"' and 姓名=N'"+sname+"'";
		ResultSet rs=query.executeQuery(queryString);
		int flag=0;
		out.println("<h2>查询学号为"+sno+"，姓名为“"+sname+"”的学生信息如下表：</h2>");
		...
		try {
			...
			if(flag==1){
				...
				out.println("<script>alert('查询失败！不存在学号为"+sno+"，姓名为“"+sname+"”的学生！')</script>");
