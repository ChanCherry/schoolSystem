...
public class UserReg_servlet extends HttpServlet{
	protected void doPost(HttpServletRequest req,HttpServletResponse resp) throws ServletException, IOException{ 
		...
		String n1="";//从数据库获取的电话和密码
		int flag=1,tmp=0;
		try {
			while(rs.next()){
				n1=rs.getString("账号");
				flag=0;
				if(n1.equals(num)){//和数据库里的匹配，注册失败，返回注册页面
					out.print("<script>alert('注册失败！该用户已存在!');</script>");
			...
			if(flag==1){
				...
				if(tmp>0){//注册成功，前往登录
					out.print("<script>alert('注册成功！即将前往登录！');</script>");
