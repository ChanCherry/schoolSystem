...
public class AmoTeaSelect2_servlet extends HttpServlet{
	protected void doPost(HttpServletRequest req,HttpServletResponse resp) throws ServletException, IOException{ 
		...
		String queryString="select * from Teacher where 姓名 like N'_"+last2+"'";
		ResultSet rs=query.executeQuery(queryString);
		int flag=0;
		out.println("<h2>查询两个字名字以“"+last2+"”字结尾的教师信息如下表：</h2>");
		...
		try {
			...
			if(flag==1){
				...
				out.println("<script>alert('查询失败！不存在两个字名字以“"+last2+"”字结尾的教师！')</script>");
